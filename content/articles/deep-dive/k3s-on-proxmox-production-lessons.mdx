...
K3s uses Flannel for CNI (default). Traefik 2.x as ingress controller (bundled with K3s, not swapped out).
...
The bundled Traefik v2 uses HTTP-01 ACME challenges by default. HTTP-01 requires the domain to be publicly reachable. My K3s cluster is on an internal network. Let's Encrypt can't reach it.
